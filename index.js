...
// ⚠️ HARAP JANGAN RECODE BAGIAN INI, GAMPANG ERROR KALO SALAH RECODE!
// ⚠️ RECODE DENGAN HATI HATI, BIAR GK ERROR!
...
async function startHisoka() {
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        m = smsg(hisoka, mek)
        require("./session.json")(hisoka, m, chatUpdate)
        } catch (err) {
            console.log(err)
        }
    })
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    ...
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Leaving from ${metadata.subject}` })
                }
            }
        } catch (err) {
            console.log(err)
